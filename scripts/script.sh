...
echo
echo "Add New Org Application Script."
echo
...
verifyResult () {
	if [ $1 -ne 0 ] ; then
		echo "Trace "$2" "
    echo "ERROR - FAILED to execute Add New Org End to End Application."
		echo
   		exit 1
	fi
}
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		...
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n samplecc -v 1.0 -c '{"Args":[""]}' -P " OR('Org1MSP.member','Org2MSP.member')" >&instantiatelog.txt
	fi
	res=$?
